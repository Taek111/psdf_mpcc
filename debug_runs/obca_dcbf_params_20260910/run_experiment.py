...
method = sys.argv[1]
folder = ROOT / method
assert method == "obca", "This runner is for the OBCA-only retest."
source_file = PROJECT / "control/obca_optimizer.py"
source_bytes = source_file.read_bytes()
source_hash = hashlib.sha256(source_bytes).hexdigest()
assert source_bytes == (ROOT / "obca_optimizer_snapshot.py").read_bytes()
(ROOT / "source_diff.patch").write_bytes(subprocess.check_output(
    ["git", "diff", "--", "control/obca_optimizer.py"], cwd=PROJECT))
...
from control.obca_optimizer import OBCAOptimizerParam

shared_dcbf = vars(NmpcDcbfOptimizerParam())
shared_obca = vars(OBCAOptimizerParam())
shared_keys = sorted(set(shared_dcbf) - {"horizon_dcbf"})
alignment = {key: bool(np.array_equal(shared_dcbf[key], shared_obca[key]))
             for key in shared_keys}
assert all(alignment.values()), alignment
(ROOT / "parameter_alignment.json").write_text(json.dumps(dict(
    shared_parameters_equal=alignment, dcbf_obstacle_horizon=shared_dcbf["horizon_dcbf"],
    obca_obstacle_horizon=shared_obca["horizon_dcbf"]), indent=2))

config = yaml.safe_load((PROJECT / "config/config.yaml").read_text())
config.update(output_root_dir=str(folder), output_suffix="dcbf_params_20260910")
