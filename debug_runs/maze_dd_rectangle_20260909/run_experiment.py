...
cli = ["--single", "--config", str(folder / "config.yaml"), "--maze-type", "maze", "--robot-shape", "rectangle", "--dynamics-type", "differential_drive", "--optimizer-type", method, "--path-planner", "astar", "--simulation-time", "60", "--no-perturb-start", "--no-animation"]
...
def capture_and_cleanup(sim):
    try:
        if sim.robot is not None:
            ...
            captured.update(initial_pose=sim.initial_pose, final_state=robot._system.get_state(), final_time=robot._system._time,
