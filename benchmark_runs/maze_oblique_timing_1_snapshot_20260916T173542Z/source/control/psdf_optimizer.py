...
class PSDFOptimizerParam:
    def __init__(self):
        ...
        self.gamma_s = 4.37e-4 # (1/2)*(r_max_robot)*(rho_{theta,s})
...
class PSDFOptimizer:
    ...
    def _compute_build_signature(self):
        ...
        tracked_files = {
            ...
            "models/psdf_wrapper.py": os.path.join(project_root, "models", "psdf_wrapper.py"),
            "models/psdf.py": os.path.join(project_root, "models", "psdf.py"),
        }
    ...
    def add_obstacle_avoidance_constraint(self, param, system, obstacles_geo):
        ...
        if self.ped_model is not None and self.ocp is not None:
            ...
            constraint_expr = sdf_value - param.d_safe - param.gamma_s
    ...
    def solve_nlp(self):
        ...
        if status != 0:
            # Return the solver status through AcadosSolution so the shared
            # trial failure checker can enforce the consecutive-failure limit.
            print(f"PSDF acados solver failed with status {status}")
