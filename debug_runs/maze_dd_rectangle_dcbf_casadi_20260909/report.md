...
|---|---|
| 상태 | timeout / timeout |
| 시뮬레이션 시간 / 적용 입력 수 | 60.0 s / 600회 |
| 최종 자세 | (0.515261, 0.542402, 0.284977) |
| A* 종점까지 남은 거리 | 1.378540 m |
| 실제 궤적 최소 외곽 간격 | 0.112375 mm |
| 기록된 상태의 접촉/겹침 | 0회 |
| 솔버 실패 | 0회 |
| 마지막 5초 이동량 | 0.000e+00 m |
| 전체 제어 호출 중앙값 / P95 | 757.9 / 909.1 ms |
| 최적화 구간 중앙값 / P95 | 386.4 / 477.2 ms |
| 실제 실행 시간 | 469.2 s |
...
2. 기본 설정의 최종 상태 재실행은 첫 입력 v=1.870e-17, omega=-1.420e-16로 사실상 정지한다. 다음 단계에는 후진·회전을, 안전 제약이 끝난 뒤에는 장애물 내부를 통과하는 전진을 계획한다. 실제로는 첫 입력만 적용하고 다시 풀기 때문에 동일한 상태와 계획을 반복한다. 서로 다른 후진·회전 초기 추정도 같은 정지 해로 수렴했으므로 초기화만의 문제로 설명하기 어렵다.
3. 원인을 분리하기 위해 단일 최적화 진단에서 horizon=11은 유지하고 horizon_dcbf만 11로 늘렸다. 첫 입력은 v=0.012612 m/s, omega=-0.500000 rad/s로 바뀌었고, 예측 궤적의 접촉/겹침 단계는 []였다. 최대 제약 위반은 9.97e-09였으며 솔버는 Solve_Succeeded를 반환했다. 이 설정으로 전체 maze 완주를 확인한 것은 아니다. 서로 제약이 다른 문제의 비용을 우열 비교에 사용하지 않았다.
